...
type AIFunction = 'generate-text' | 'generate-descriptions' | 'generate-axes' | 'sum-letters';
  ...
  const generateContent = async () => {
    ...
    try {
      ...
      if (selectedFunction === 'generate-text') {
        ...
        setGeneratedContent(data.sums.join(', '));
      }
    } catch (error) {
  ...
  const handleVerify = async () => {
    ...
    try {
      await submitJeux({
        ...
        bbox: selectedFunction === 'sum-letters' 
          ? generatedContent.split(', ').map(Number)
          : [generatedContent],
      });
